...
__device__ bool wouldHitAsteroid(const Simulation& sim, uint8_t fireAngle) {
    const Ship& ship = sim.ship;
    int32_t targetIdx = -1;
...
__device__ bool verifyShotFarm(const Simulation& sim, uint8_t fireAngle) {
    const Ship& ship = sim.ship;
    int32_t absVx = ship.vx < 0 ? -ship.vx : ship.vx;
    int32_t absVy = ship.vy < 0 ? -ship.vy : ship.vy;
    int32_t shipSpeedApprox = ((absVx + absVy) * 3) >> 2;
    ...
    for (int step = 0; step < 24; step++) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            int32_t hitDist = (2 + sim.saucers[i].radius + 4) << 4; // generous +4px tolerance
...
__device__ bool bulletApproachingSaucer(const Simulation& sim, int bi, int si) {
...
__device__ uint8_t farm(const Simulation& sim) {
    ...
    for (int bi = 0; bi < SHIP_BLIMIT; bi++) {
        ...
        for (int si = 0; si < 3; si++) {
            ...
            if (bulletApproachingSaucer(sim, bi, si)) {
                covered[si] = true;
            }
        }
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // Enables faster target cycling after a kill
        if (sim.saucerCount >= 2) {
            ...
            if (minGap <= 20) {
                time -= (int64_t)(20 - minGap) * 2; // up to -40 bonus
            }
        }
        if (time < best) {
            best = time;
    ...
    if (id >= 0) {
        ...
        for (int iter = 0; iter < 3; iter++) {
            int32_t dist = (int32_t)__fsqrt_rn((float)((int64_t)pdx * pdx + (int64_t)pdy * pdy));
        ...
        if (ship.fireCooldown == 0 && sim.bulletCount < SHIP_BLIMIT) {
            ...
            bool canFire = (delta <= 7) ||
                           (delta <= 15 && verifyShotFarm(sim, fireAngle));
            // Extra: try current angle (before turn) for opportunistic hits
            // This catches cases where the ship is aimed at a different saucer
            if (!canFire && dir != 0 && verifyShotFarm(sim, (uint8_t)ship.angle)) {
                canFire = true;
                fireAngle = (uint8_t)ship.angle;
                inp &= ~(INPUT_LEFT | INPUT_RIGHT); // don't turn, fire at current angle
            }
            if (canFire && !wouldHitAsteroid(sim, fireAngle)) {
                inp |= INPUT_FIRE;
            }
        }
    }
